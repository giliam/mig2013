...
La requête analysée par le SpeechServer, les opérations adéquates ayant été réalisées par le coeur algorithmique, le SpeechServer répond au client par une réponse HTTP POST\footnote{HTTP est un protocole de communication client-serveur développé pour web (Wikipédia)} contenant des données au format XML\footnote{XML est un langage informatique de balisage facilitant l'échange automatisé de contenus complexes (Wikipédia)}.
