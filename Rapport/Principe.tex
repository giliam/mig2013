...
La première étape pour faire de la reconnaissance vocale est de parvenir à trouver un moyen de caractériser efficacement et uniformément un mot. Cela signifie désigner un mot par un certain motif puis permet par le même procédé
appliqué sur un enregistrement quelconque, de parvenir à identifier le motif le plus vraisemblable qui correspondrait alors au même mot.
...
Afin de gérer ces difficultés, nous avons mis en place plusieurs étapes successives de traitement supplémentaire afin d'obtenir cette fameuse \og trace \fg qui caractériserait un enregistrement, c'est-à-dire un mot. Nous avons pour cela 
utilisé plusieurs techniques de traitement du signal communément reconnues (échantillonnage, fenêtrage, transformée de Fourier directe et inverse).
Cette figure explique globalement le traitement que nous avons choisi de mettre-en-place afin de reconnaître le mot prononcé. Il y a donc plusieurs étapes qui s'enchaînent pour parvenir à un objet que nous pourrons manipuler en le sachant représentatif et caractéristique du son.
\begin{figure}[!h]
	    \begin{center}
...
La première étape consiste simplement à enregistrer le son sur le disque dur de l'ordinateur. Nous utilisons pour cela le module intégré à Python appelé PyAudio\cite{pyaudio}. Cela permet d'enregistrer
avec une certaine \emph{fréquence d'échantillonnage} (donc un certain nombre de captures de son par seconde) les amplitudes du son captées par le micro.
...
Jusque là, le son étudié représentait temporellement ce qui avait été entendu. Néanmoins, il est difficile d'étudier un son tel quel et on utilise alors le lien
